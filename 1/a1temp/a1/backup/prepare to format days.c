...
void get_events(char *file){
	...
	char *p = NULL;
	char str[MAX_LINE_LEN], temp_char_a, temp_char_b;
...
void update_events(char *start, char *end){
	...
	char temp[MAX_LINE_LEN];
  	for(i = 0; i < num_entries; i++){
		if(strncmp(entry[i].until, "20", 2) == 0){
...
void print_events(char *start, char *end){
	...
	int k, first_in_all, temp_num, temp_num1;
	char date[MAX_LINE_LEN], underline[MAX_LINE_LEN];
	char temp[MAX_LINE_LEN], time_str_start[MAX_LINE_LEN], time_str_end[MAX_LINE_LEN];
	first_in_all = 1;
	...
	while (strncmp(start, end, 8) <= 0){		
		...
		for(k = 0; k < num_entries; k++){	
			...
			if(strncmp(entry[k].dt_start, start, 8) == 0){
				...
				if(strncmp(entry[k].until, "20", 2) != 0){
					...
					dt_increment(temp, entry[k].dt_start, 7);
					strncpy(entry[k].dt_start, temp, MAX_LINE_LEN);
					sscanf(entry[k].dt_start,"%dT%d", &temp_num, &temp_num1);
				}
			}	
		}
		dt_increment(temp, start, 1);
