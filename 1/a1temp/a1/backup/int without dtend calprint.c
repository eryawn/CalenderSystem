	...
	p = fgets(str, MAX_LINE_LEN,file_pointer);
		while(strncmp(str, "END:VCALENDAR", 13) != 0){	
			...
			while(strncmp(str, "END:VEVENT", 10) != 0){	
				...
				if (strncmp(str, "RRULE:", 6) == 0) {
					sscanf(str,"RRULE:FREQ=WEEKLY;WKST=MO;UNTIL=%dT%d;BYDAY=[A-Z][A-Z]", 
											&entry[num_entries].until, &temp_num, &temp_char_a, &temp_char_b);
					entry[num_entries].until += 1;
				}
				else if (strncmp(str, "DTSTART:", 8) == 0) {
					sscanf(str,"DTSTART:%s", &entry[num_entries].dt_start);
					sscanf(str,"DTSTART:%dT%d", &entry[num_entries].day, &entry[num_entries].hour_start);
				}
	...
	for(i = 0; i < num_entries; i++){
		...
		if(entry[i].until > 0){
			...
			while(entry[i].day < start){
				dt_increment(entry[i].dt_start, entry[i].dt_start, 7);
		...
		for(k = 0; k < num_entries; k++){	
			...
			if(entry[k].day == start){			
				...
				for(j = 0; j < strlen(date) + 1; j++){
				 ...
				 //for those events that are not repeated
				if(entry[k].until == 0){		
				...
				if(entry[k].until > 0 && start < entry[k].until){		
					...
					dt_increment(entry[k].dt_start, entry[k].dt_start, 7);	
